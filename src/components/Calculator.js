...
const Calculator = () => {
  ...
  //handled every button click except sum and  clear
  const handleClick = (e) => {
    setDisplay(display.concat(e.target.name));
  };

  // for clearing results
  const handleClear = (e) => {
    setDisplay("");
  };

  //to remove last element from result
  ...
  const handleEval = () => {
    ...
    try {
      ...
      const errorMessage = "Error! Please Enter Inputs..";
  ...
  return (
    <>
      <div className={Styles.heading}>My Calculator</div>
